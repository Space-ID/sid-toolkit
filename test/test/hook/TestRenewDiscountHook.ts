...
describe("DefaultDiscountHook test", function () {
    ...
    async function deployFixture() {
        ...
        const renewFee = toBigInt("158443692534");
        //deploy renew discount hook
        const discountHook = await ethers.deployContract("RenewDiscountHook",
            [sann, identifier, preRegiState, platformConfig, giftCardLedger, priceOracle, preRegiDiscountRateBps, publicRegistrationStartTime, renewFee]);
        await discountHook.waitForDeployment();

        await sann.connect(platformAdmin).setTldController(mockController);
        await registrar.connect(tldOwner).setPriceHook(identifier, discountHook);
        await registrar.connect(tldOwner).setRenewPriceHook(identifier, discountHook);
        await registrar.connect(tldOwner).setPointHook(identifier, discountHook);

        await giftCardLedger.connect(tldOwner).addTldGiftCardController(identifier, discountHook);
