...
async function deployToolkit(
    ...
    // registry
    const registry = await ethers.deployContract("SidRegistry", [
        signer[0].address,
    ]);
    ...
    const controllerProxy = await ethers.deployContract("ERC1967Proxy", [
        platformAdmin,
        keccak256(toUtf8Bytes("ControllerProxy")),
    ]);
