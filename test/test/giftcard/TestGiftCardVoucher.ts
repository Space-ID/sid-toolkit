...
describe("GiftCardVoucher test", function () {
    ...
    it("should only allow tldOwner to add new customized voucher", async function () {
        ...
        let tokenId = await voucher
            .connect(tldOwner)
            .addCustomizedVoucher.staticCall(identifier, setPrice);
        // reverts since caller is not the tldOwner
        await expect(
            voucher.connect(addr1).addCustomizedVoucher(identifier, setPrice)
        ).to.be.revertedWith("Ownable: caller is not the tld owner");

        await expect(
            voucher.connect(tldOwner).addCustomizedVoucher(identifier, setPrice)
        )
    ...
    it("should summarize total value of giftcards", async function () {
        ...
        let tokenId2 = await voucher
            .connect(tldOwner)
            .addCustomizedVoucher.staticCall(identifier, setPrice2);
    ...
    it("should check if the giftcards belong to same TLD or not", async function () {
        ...
        let tokenId3 = await voucher
            .connect(tldOwner2)
            .addCustomizedVoucher.staticCall(identifier2, setPrice3);
